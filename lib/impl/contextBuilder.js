...
const parseResultsToUnions = (distinct) => (list, data) => (list.push(utils_1.newUnion(distinct, data)), list);
exports.newQueryContextBuilder = () => {
    ...
    const handleJoin = (type) => (...args) => {
        ...
        tempJoinBuilder = utils_1.newJoinBuilder(type, defualtTable, args[0]);
    };
    const handleOn = (...args) => {
        if (!tempJoinBuilder) {
